...
[Part 2: using Socratic to improve the performance of a vertical agent.](https://kevins981.github.io/blogs/teachagent_part2.html)
...
To reliably perform highly speciailized tasks, human experts use intuitions, best practices, heuristics built over accumulated experiences.
<!-- 
...
However, transferring this expert knowledge from humans to agents is not easy. This knowledge is often fragmented and unstructured. More importantly, domain expertise requires deep understanding of principles, heuristics, and edge cases beyond memorizing facts. So **how can we transfer such knowledge from human to agent?**
...
Two common approaches to transfer knowledge are prompt engineering and infomation retrieval.
...
Second, writing system prompts demands that experts enumerate their knowledge upfront. But human expertise is often tacit and difficult to articulate (["We can know more than we can tell"](https://en.wikipedia.org/wiki/Polanyi%27s_paradox)).
...
- **Iterative:** Teaching happens across mulitple sessions. The student's knowledge evolves over time.
...
The inputs to the system are source documents. In practice, this can be anything that contains domain knowledge: design docs, code, meeting notes, daily logs, past chat transcripts etc. These are things that you previous would have put into a RAG/document dump.
Teaching happens through _chat sessions_ between the human user and agent student.
...
Following our insight about effective teaching (interactive), Socratic implements two methods to initiate knowledge transfer:
1. **Teacher-initiated teaching.** The human user instructs the agent a topic to learn. E.g. "Let's look at how Socratic stores the knowledge base." The agent studies the relevant documents and proposes updates to the knowledge base. The human reviews, corrects, and approves these updates. 
...
Socratic is naturally iterative. The student agent updates the knowledge base over each chat session. As the knowledge base evolves, the student agent's understanding of the doamin evolves as well. 
...
Of course, while this vision is exciting, it is unclear how much performance we can squeenze out of existing agents throught teaching.
In Part 2 of this blog series, I will share a concrete use case of Socratic, optimizing an airline customer service agent, and evaluate its effectiveness. 
